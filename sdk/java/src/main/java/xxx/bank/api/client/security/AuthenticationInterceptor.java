package xxx.bank.api.client.security;
...
public class AuthenticationInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        boolean isApiKeyRequired = original.header(BankxxxConstants.ENDPOINT_SECURITY_TYPE_APIKEY) != null;
        boolean isSignatureRequired = original.header(BankxxxConstants.ENDPOINT_SECURITY_TYPE_SIGNED) != null;
        newRequestBuilder.removeHeader(BankxxxConstants.ENDPOINT_SECURITY_TYPE_APIKEY)
                .removeHeader(BankxxxConstants.ENDPOINT_SECURITY_TYPE_SIGNED);

        // Endpoint requires sending a valid API-KEY
        if (isApiKeyRequired || isSignatureRequired) {
            newRequestBuilder.addHeader(BankxxxConstants.API_KEY_HEADER, apiKey);
        }
