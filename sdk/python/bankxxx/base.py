...
from . exceptions import BankxxxAPIException, BankxxxRequestException
...
class Request(object):
    ...
    def __init__(self, api_key, secret, entry_point='https://api.bank.xxx/openapi/', proxies=None):
    ...
    def _handle_response(self, response):
        ...
        if not str(response.status_code).startswith('2') and not response.status_code == 400:
            raise BankxxxAPIException(response)
        try:
            return response.json()
        except ValueError:
            raise BankxxxRequestException('Invalid Response: %s' % response.text)
