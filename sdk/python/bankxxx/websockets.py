...
from .exceptions import BankxxxRequestException
from . client import BankxxxClient


class BankxxxClientProtocol(WebSocketClientProtocol):
...
class BankxxxReconnectingClientFactory(ReconnectingClientFactory):
...
class BankxxxClientFactory(WebSocketClientFactory, BankxxxReconnectingClientFactory):
    ...
    protocol = BankxxxClientProtocol
    ...
    def buildProtocol(self, addr):
        return BankxxxClientProtocol(self, payload=self.payload)


class BankxxxSocketManager(threading.Thread):

    def __init__(self, api_key='', secret='', entry_point='wss://wsapi.bank.xxx/openapi/', auth=True, rest_entry_point='https://api.bank.xxx/openapi/'):
        ...
        if auth:
            self._client = BankxxxClient(api_key=self._api_key, secret=self._secret, entry_point=rest_entry_point) if api_key and secret else None
    ...
    def _start_socket(self, id_, path, payload, callback):
        ...
        factory = BankxxxClientFactory(factory_url, useragent='Bankxxx-P 1.0', payload=payload)
        factory.base_client = self
        factory.protocol = BankxxxClientProtocol
    ...
    def _start_auth_socket(self, id_, payload, callback):
        ...
        if not listen_key:
            raise BankxxxRequestException('Get listen key failure.')
...
class BankxxxWss(BankxxxSocketManager):
